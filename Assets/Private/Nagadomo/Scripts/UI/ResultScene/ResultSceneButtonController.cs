...
public class ResultSceneButtonController : MonoBehaviour
{
    public static readonly Vector3[] SelectorPositions = new Vector3[]
    {
        new Vector3(449.0f, -237.0f, 0),
        new Vector3(449.0f, -353.0f, 0)
    };
    ...
    private void Awake()
    {
        _uiGridSelector = new UIGridSelector(2, 1);
    ...
    void Start()
    {
        ...
        for (int i = 0; i < _buttonBases.Length; i++)
        {
            // 選択中のボタンをオンにする
            if (i == _currentIndex)
                _buttonBases[i].ChangeAnimationState(_buttonBases[i].GetAnimationState<OnResultSceneButtonAnimationState>());
            else
                _buttonBases[i].ChangeAnimationState(_buttonBases[i].GetAnimationState<OffResultSceneButtonAnimationState>());
        }
